...
void PortM_Init(void){
	...
	GPIO_PORTM_DIR_R |= 0;        								// make PN0 out (PN0 built-in LED1)
  GPIO_PORTM_AFSEL_R &= ~0x0F;     								// disable alt funct on PN0
  GPIO_PORTM_DEN_R |= 0x0F;        								// enable digital I/O on PN0
																									// configure PN1 as GPIO
  //GPIO_PORTN_PCTL_R = (GPIO_PORTN_PCTL_R&0xFFFFFF0F)+0x00000000;
  GPIO_PORTM_AMSEL_R &= ~0x0F;     								// disable analog functionality on PN0		
	return;
}
...
void button_2() {
	if((GPIO_PORTM_DATA_R & 0x02) == 0x02) {
		while((GPIO_PORTM_DATA_R & 0x02) == 0x02) {}
		UART_printf("@");
		FlashLED1(1);
	}
}
...
//  status = VL53L1X_SetInterMeasurementInMs(dev, 200); /* in ms, IM must be > = TB */
  ...
  status = VL53L1X_StartRanging(dev);   // This function has to be called to enable the ranging
	...
	while(1) {
		button_2();
		if((GPIO_PORTM_DATA_R & 0x01) == 0x01) {
			while((GPIO_PORTM_DATA_R & 0x01) == 0x01) {}
			FlashAllLEDs();
			for(int i = 0; i < 512; i++) {
